namespace CarShop.Services
{
    ...
    public class Validator : IValidator
    {
        public ICollection<string> ValidateUser(RegisterUserFormModel model)
        {
            ...
            if (model.Password.All(x => x == ' '))//da nqma prazno mqsto
            {
        ...
        public ICollection<string> ValidateCar(AddCarFormModel model)
        {
            ...
            {
                errors.Add($"Car '{model.Model}' is not valid. It must be between {CarModelMinLength} and {DefaultMaxLength} characters long.");
            }

            if (model.Year < CarYearMinValue || model.Year > CarYearMaxValue)
            {
                errors.Add($"Car '{model.Model}' is not valid. It must be between {CarYearMinValue} and {CarYearMaxValue} characters long.");
            }
            ...
            {
                errors.Add($"Plate number {model.Image} is not valid. It should be in format 'AA0000AA'.");
            }
